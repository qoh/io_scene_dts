...
def save_from_armature(operator, context, filepath,
         select_object=False,
         select_marker=False,
         blank_material=True,
         generate_texture="disabled",
    ...
    def export_bones(bones, parent):
        for bone in bones:
            ...
            node.translation = mat.to_translation()
            node.rotation = mat.to_rotation()

            shape.nodes.append(node)

            bone_lookup[bone] = node
            bone_name_table[bone.name] = node
            
            export_bones(bone.children, bone)

    export_bones(filter(lambda b: not b.parent, armature.data.bones))
    ...
    for bobj in scene.objects:
        ...
        if armature_modifier is None:
            return fail(operator, 'Only armature deform meshes are supported in armature mode for now')

        if auto_root_index is None: # need to find proper root somehow
            auto_root_index = len(shape.nodes)
            shape.nodes.append(Node(shape.name("__auto_root__")))
            shape.default_rotations.append(Quaternion((1, 0, 0, 0)))
            shape.default_translations.append(Vector())
    ...
    for object, lods in scene_objects.values():
        ...
        for i in range(object.numMeshes):
            ...
            if lod_name in lods:
                ...
                dmesh = Mesh(Mesh.SkinType)
                dmesh.initialVerts = dmesh.verts
                dmesh.initialNormals = dmesh.normals
                dmesh.initialENormals = dmesh.enormals
                dmesh.initialTransforms = []
                dmesh.vertexIndex = []
                dmesh.boneIndex = []
                dmesh.weight = []
                dmesh.nodeIndex = []
                ...
                for material_index, polys in grouped_polys:
                    ...
                    for poly in polys:
                        ...
                        for vert_index, loop_index in zip(reversed(poly.vertices), reversed(poly.loop_indices)):
                            ...
                            # only do this for skinned meshes?
                            influences = []

                            for group in vert.groups:
                                if group.group > len(bobj.vertex_groups):
                                    continue

                                vertex_group = bobj.vertex_groups[group.group]
                                node = bone_name_table.get(vertex_group.name)
                                if not node:
                                    continue
                            ...
                            def get_vertex_bone(node):
                                for bone_index, node_index in enumerate(dmesh.nodeIndex):
                                    if node_index == node.index:
                                        return bone_index

                                bone_index = len(dmesh.nodeIndex)
                                mat = node.bone.matrix_local
                                data = mat.row
                                flat = [x for y in data for x in y]

                                self.nodeIndex.append(node.index)
                                self.initialTransforms.append(flat)
                            ...
                            for node, weight in influences
                                self.vertexIndex.append(vertex_index)
                                self.boneIndex.append(get_vertex_bone(node))
                                self.weight.append(weight / total)
